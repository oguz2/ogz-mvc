...
if(!isset($path["appPath"]) OR !is_dir(realpath($path["appPath"])) )
{
	...
	{
		$path["appPath"] =  realpath(realpath("Public")).DIRECTORY_SEPARATOR;
	}
	else
	{
...
{
	$path["appPath"] = realpath($path["appPath"]).DIRECTORY_SEPARATOR;
}
...
if(!isset($path["configPath"]) OR !is_dir(realpath($path["configPath"])) )
{
	...
	{
		$path["configPath"] =  realpath(realpath("Config")).DIRECTORY_SEPARATOR;
	}
	else
	{
		header('HTTP/1.1 503 Service Unavailable.', TRUE, 503);
...
{
	$path["configPath"] = realpath($path["configPath"]).DIRECTORY_SEPARATOR;
}
...
if(!isset($path["systemPath"]) OR !is_dir(realpath($path["systemPath"])) )
{
	...
	{
		$path["systemPath"] = realpath("System").DIRECTORY_SEPARATOR;
...
{
	$path["systemPath"] = realpath($path["systemPath"]).DIRECTORY_SEPARATOR;
}
...
if(!isset($path["controllerPath"]) OR !is_dir(realpath($path["controllerPath"])) )
{
	...
	{
		$path["controllerPath"] = realpath(realpath($path["appPath"]. "Controller")).DIRECTORY_SEPARATOR;
	}
	else
	{
		header('HTTP/1.1 503 Service Unavailable.', TRUE, 503);
...
{
	$path["controllerPath"] = realpath($path["controllerPath"]).DIRECTORY_SEPARATOR;
}
...
if(!isset($path["viewPath"]) OR !is_dir(realpath($path["viewPath"])) )
{
	...
	{
		$path["viewPath"] = realpath($path["appPath"]. "View").DIRECTORY_SEPARATOR;
	}
	else
	{
		header('HTTP/1.1 503 Service Unavailable.', TRUE, 503);
...
{
	$path["viewPath"] = realpath($path["viewPath"]).DIRECTORY_SEPARATOR;
}
...
if(!isset($path["model"]) OR !is_dir(realpath($path["model"])) )
{
	...
	{
		$path["model"] = realpath($path["appPath"]. "Model").DIRECTORY_SEPARATOR;
	}
	else
	{
		header('HTTP/1.1 503 Service Unavailable.', TRUE, 503);
...
{
	$path["model"] = realpath($path["model"]).DIRECTORY_SEPARATOR;
}
...
if(!isset($path["logPath"]) OR !is_dir(realpath($path["logPath"])) )
{
	...
	{
		$path["logPath"] = realpath($path["logPath"]).DIRECTORY_SEPARATOR;
	}
	else
	{
		header('HTTP/1.1 503 Service Unavailable.', TRUE, 503);
...
if(!isset($path["libraryPath"]) OR !is_dir(realpath($path["libraryPath"])) )
{
	...
	{
		$path["libraryPath"] = realpath($path["libraryPath"]).DIRECTORY_SEPARATOR;
	}
	else
	{
		header('HTTP/1.1 503 Service Unavailable.', TRUE, 503);
...
if(!isset($path["lang"]) OR !is_dir(realpath($path["lang"])) )
{
	...
	{
		$path["lang"] = realpath($path["lang"]).DIRECTORY_SEPARATOR;
	}
	else
	{
		header('HTTP/1.1 503 Service Unavailable.', TRUE, 503);
